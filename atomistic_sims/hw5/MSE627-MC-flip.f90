...
module MC_procedures
    ...
    real, parameter :: T = 16742.0                    ! Temperature [K]
    ...
    real, parameter :: E(n_interactions) = (/-0.25, 0.00, 0.00/) ! Energy of AA, BB, AB interactions [eV]
    ...
    subroutine swap(id1)
        ...
        types(id1) = mod(types(id1), 2) + 1 ! Change atom type rather than swapping positions; (1%2) + 1 = 2; (2%2) + 1 = 1 as clever way to change types
...
!   Distributing initial atom types at random
    do i = 1, Natoms
        ...
        if(rnd > 0.6) then ! start with 60% of non-vacancies
            types(i) = 1
        else
            types(i) = 2
        endif
...
!   Metropolis method starts here *************************
    do step = 0, Nsteps
!       Choosing one atoms to flip (N1)
        do
            call random_number(rnd)
            N1 = int(rnd*Natoms) + 1
            exit ! No need for a fancy conditions
        enddo
!       Calculating energy and number of affected bonds before the swap
        E_i = get_E(N1, Nbonds_tmp)
        Nbonds_i(:) = Nbonds_tmp(:)
!       Flip the atom identity
        call swap(N1)
