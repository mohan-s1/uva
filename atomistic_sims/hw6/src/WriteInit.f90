...
!   Counting particles of different types
    ...
    write(17,"('KFLAG = ',I1,' (1-Quench, 2-Vel ....)')") KFLAG
    write(17,"('LFLAG = ',I1,' (1-Constant Pressure)')") LFLAG
    write(17,"('KEYBS = ',I1,' (0-pair potential, 1-Stillinger Weber, 2-EAM, 3-Tersoff)')") KEYBS
    write(17,"('Boundary(1) = ',I1,' (1-periodicity in X direct.,0-free boundaries)')") Boundary(1)
    write(17,"('Boundary(2) = ',I1,' (1-periodicity in Y direct.,0-free boundaries)')") Boundary(2)
    write(17,"('Boundary(3) = ',I1,' (1-Z PBC,0-Z free or boundary layer)')") Boundary(3)
    write(17,"('KBOUND = ',I2,' (0-Free,1-rigid,2-...)')") KBOUND
    ...
    write(17,*) "   -----> MATERIAL:"
    write(17,"('CellSize(1) = ',D12.5,' (X size of the computational cell [A])')") CellSize(1)
    write(17,"('CellSize(2) = ',D12.5,' (Y size of the computational cell [A])')") CellSize(2)
    write(17,"('CellSize(3) = ',D12.5,' (Z size of the computational cell [A])')") CellSize(3)
    if(dim == 2) then
        write(17,"('DENSM = ',D12.5,' (Density of the material [gm/cm2])')") DENSM
        write(17,"('DENSN = ',D12.5,' (Density of the material [molec./cm2])')") DENSN
    else
        write(17,"('DENSM = ',D12.5,' (Density of the material [gm/cm3])')") DENSM
        write(17,"('DENSN = ',D12.5,' (Density of the material [molec./cm3])')") DENSN
    endif
    ...
    write(17,"('Type ',I2,': ',I5,' particles.  mass = ',D12.5,' [amu].')") (i, KTT(i), mass(i), i = 1, NTYPE)
    write(17,*)
