...
module GlobalVars
    ...
    real*8, allocatable :: XD(:,:), FD(:,:), DR(:,:), R_OLD(:,:) !Coordinates, forces, dispalcement, array to hold positions before calling integrator
...
contains
    subroutine allocate_system()                    !allocate memory for arrays used in the simulation according to Natoms
        ...
        if(allocated(DR)) deallocate(DR) ! this is new
        if(allocated(R_OLD)) deallocate(R_OLD) ! this is new
        ...
        allocate(XD(3,Natoms), FD(3,Natoms), DR(3, Natoms), R_OLD(3, Natoms), Q1D(3,Natoms), Q2D(3,Natoms), Q3D(3,Natoms), Q4D(3,Natoms), Q5D(3,Natoms)) ! added DR and R_OLD here
