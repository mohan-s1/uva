...
!   Pair potentials and forces are tabulated
    ...
    DR(3,Natoms) = 0.0d0 ! initialize displacement array DR with all 0s; note that memory for DR is allocated in GlobalVars.f90

!   Beginning of MD loop
    do step = 0, NSTEP
!       Updating Neighbor List
        if(mod(step,NEWTAB) == 0) call NbList()
        R_OLD = XD ! save positions before calling integrator; note that memory for R_OLD is allocated in GlobalVars.f90
        call Nord5()                ! Integration
!       Gathering all particles to the initial cell
        DR = DR + (XD - R_OLD) ! update displacement array
...
!       Printing output information
        if(mod(step,NEPRT) == 0) then
            ...
            write(17,"(I6,6(1X,E15.8))") step, time, E_tot, E_k, E_p, T, P_tot
            flush(17)
        endif
